//! Lifecycle operations (start, stop, delete, wipe) for Darwin VMs.
...
pub async fn start_darwin_instance(paths: &ResearchPaths, id: &str) -> Result<()> {
    ...
    if !ready {
        let _ = supervisor_handle
            ...
            .await;
        bail!("Supervisor failed to achieve readiness for Darwin VM '{id}' within deadline");
    }
...
pub async fn stop_darwin_instance(paths: &ResearchPaths, id: &str) -> Result<()> {
...
pub async fn delete_darwin_instance(paths: &ResearchPaths, id: &str) -> Result<()> {
    let _ = stop_darwin_instance(paths, id).await;
    delete_instance(paths, id).await?;
    Ok(())
}

pub async fn wipe_darwin_instance(paths: &ResearchPaths, id: &str) -> Result<()> {
    stop_darwin_instance(paths, id).await?;
