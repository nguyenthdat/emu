//! Darwin minimal virtual machine manager implementation.
...
pub struct DarwinVmManager {
    paths: Arc<ResearchPaths>,
    executor: Arc<dyn CommandExecutor>,
}

impl DarwinVmManager {
...
impl DeviceManager for DarwinVmManager {
    ...
    fn delete_device(&self, identifier: &str) -> impl Future<Output = Result<()>> + Send {
        let paths = self.paths.clone();
        let id = identifier.to_string();
        async move { lifecycle::delete_darwin_instance(&paths, &id).await }
    }

    fn wipe_device(&self, identifier: &str) -> impl Future<Output = Result<()>> + Send {
        let paths = self.paths.clone();
        let id = identifier.to_string();
        async move { lifecycle::wipe_darwin_instance(&paths, &id).await }
    }
